...
> **注**: `.steering/` を除外するかは**プロジェクト方針による**。作業単位ドキュメントを履歴として
> 残す運用なら**コミット対象にする**(本プロジェクトはこちら。`repository-structure.md`「除外設定」を
> 参照)。一時ファイルとして扱う運用なら除外する。
...
> **注**: `.steering/` をフォーマッタの除外に入れるかも**プロジェクト方針による**（上記 `.gitignore`
> の注記と同じ）。作業単位ドキュメントを成果物として整形したいならコミット対象かつ整形対象にする
> （本プロジェクトはこちら）。