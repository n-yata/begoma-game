...
> 本節は**概念モデル**（エンティティ・区分値・ドメイン上の制約・ER図）までを扱う。
> より具体・実装寄りの情報は下流を正本とする。
>
> - **論理設計**（キー設計・正規化・関連の解決・区分値の表現方法・削除/履歴方針・命名規約と
>   その理由）→ `docs/specs/3_detail-design/db/logical-data-model.md`（論理データモデル書）
> - **物理スキーマ**（カラム定義・型・NULL可否・デフォルト・物理制約名・索引名）→
>   `docs/specs/3_detail-design/db/table-definition.md`（テーブル定義書）
...
> この場でユーザーと一緒に確定させてから書く。論理設計（キー設計・正規化・削除方針）と
> 物理スキーマ（カラム定義・制約・索引）の作成は本ガイドの範囲外。baseline のデータモデルが
> 確定したら、**`/flow-design-database`（`flow-design-database` スキル）**を入口として
> `docs/specs/3_detail-design/db/logical-data-model.md`（論理）→ `table-definition.md`（物理）の
> 順に詳細化する。
