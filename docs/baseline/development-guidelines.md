...
- **物理挙動に影響する修正（コライダ形状・判定・接触検出など）は再バランスとセットで見積もる**。
  修正そのものより計測→再調整の方が重いことがある（20260820-collision-fixes の実績）

### Rapier 利用上の注意（実測で得た制約）

- **衝突強度は `world.step()` の前に速度を保持してから測る**。step 後の速度は反発で
  減衰済みのため、攻守判定・衝突減衰量が過小になる
- **衝突開始イベントを当たり判定の主軸にしない**。持続接触では再発火せず、取りこぼしもある。
  当たり判定は `world.contactPair`（接触マニフォールド）による毎ステップの実接触確認 +
  クールダウン方式を使う。イベントは「一度きりの検出」（トコ接地など）にのみ使う。
  距離ベースの近接判定（勢い負けノックバック）も同様にイベントに依存しない
- **高速衝突には CCD を有効化する**（`setCcdEnabled(true)`。すり抜け防止）
- **判定と地形は一致させる**。「物理的に立てる場所」と「場外と判定する場所」に隙間があると
  「盤外で回り続ける」類のバグになる。地形側（コライダ形状）で隙間そのものを消し、
  不変条件テスト（決着時に場外でないコマはトコ半径内）で再発を検出する
