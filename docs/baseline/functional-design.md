...
- **対戦（Match）**: フェーズ・プレイヤーのコマ・CPUのコマ・経過時間・結果（成立時のみ）
...
| リザルト | `result` | `ResultScreen` | 勝敗表示と再戦 | 勝敗・決着理由・「同じコマで再戦」「コマ選択へ」ボタン |
...
stateDiagram-v2
    ...
    battle --> result: 決着（場外/停止/失敗）
    result --> aiming: 同じコマで再戦
    result --> komaSelect: コマ選択へ
```
...
|---|---|---|---|---|
| UC-01 | 対戦を1プレイ完了する | プレイヤー | 投擲から決着までの中核体験 | タイトル→コマ選択→投擲→対戦→リザルト |
| UC-02 | コマを選んで戦い方を変える | プレイヤー | 種別ごとの個性で戦略を選ぶ | コマ選択 |
| UC-03 | 決着後すぐ再戦する | プレイヤー | テンポよく繰り返し遊ぶ | リザルト→投擲 |
